...
(function($){
    ...
    jValidation.util = {
    	...
    	getElmID : function(elm) {
    		var reg = /[\[\]\.\{\}]/g;
    		try{
    			var result = $(elm).attr("id") ? $(elm).attr("id") : $(elm).attr("name").replace(reg,"");
    			return result;
    		}catch(e){
    		}
    		return ""; 
    ...
    jValidation.Validator.messageSource['zh-cn'] = [
		...
		['validate-money','请输入正确的精度,最高保留4位小数']
    ];
    ...
    jValidation.Validation.prototype = {
    	init: function(jForm,options){
    		...
    		if(this.options.immediate) {
    			...
    			var elements =jValidation.util.getFormElements(this.jForm); 
    ...
    $.extend(true,jValidation.Validation,{
    	...
    	showErrorMsg : function(name,elm,errorMsg) {
    		...
    		var jElm = $(elm);
    		if(typeof jWei !='undefined' && typeof jWei.Tips != 'undefined' &&  jValidation.Tips.useTips){
    				jWei.Tips.tip($(elm),{content:errorMsg,position:jValidation.Tips.position,showArrow:jValidation.Tips.showArrow});
	...
	//custom validate start
	 Validation.addAllThese([
        ...
        */			
		...
		['validate-mobile-phone',/^0?(13[0-9]|15[012356789]|18[0123456789]|14[57])[0-9]{8}$/],
		['validate-zip',/^[1-9]\d{5}$/],
		['validate-qq',/^[1-9]\d{4,8}$/],
		['validate-devid',/^[0-9A-Z]{6}$/],
		['validate-space',/^[^\s]+$/],
		['validate-money',/^\d+(?:\.\d{1,4})?$/]
	]);
