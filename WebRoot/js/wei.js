...
(function(window,$,undefined) {
	...
	jWei.PopupContainer = (function(){
		var instantiated = false,
		 	  html = '<div  class="popup"><div class="popup-title">'+
	    				...
	    				'<button class="btn  btn-danger btn-ok"><i class="icon-ok  icon-white"></i>确定</button></div></div>',
	    	init = function (){
				$(document.body).append(html);
				var popUp = $(".popup"),
					  overlay = jWei.Overlay.getInstance(),
	...
	jWei.Datagrid.prototype = {
		...
		_settingSelectAll : function() {
			...
			if (this.showCheck) {
				var th = $('<th width="'
						+ this.chkWidth
						+ 'px"><label class="tab-th-check"><input type="checkbox" id="selectAll_id"/>全选</label></th>');
	...
	*/
	...
	jWei.Page.prototype = {
		...
		setSkipPage:function(){
			var page = this;
			this.ulElm.append(this.skipPage);
			
			this.skipPage.find(".skip-btn").click(function(){
				var no = page.skipPage.find(".skip-page").val();
				var reg = /^\d+$/;
				if(!reg.test(no)){
					no = 1;
					page.skipPage.find(".skip-page").val(1);
				}else{
					if(no <=0){
						no =1;
						page.skipPage.find(".skip-page").val(1);
					}else if(no >= page.getPageNum()){
						no = page.getPageNum();
						page.skipPage.find(".skip-page").val(no);
					}
				}
		...
		initPage:function(){
			...
			this.skipPage=$('<li><input class="skip-page" style="width:40px;margin-top:2px;" type="text"/><button class="skip-btn">跳转</button></li>');
			this.drawPage();
		},
